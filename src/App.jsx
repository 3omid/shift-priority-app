...
// Approximate yard coordinates, used only on-device to pre-select the
// nearest yard from the phone's GPS (nothing is sent anywhere). Only a
// yard within DAILY_LOG_YARD_RADIUS_KM counts. Stouffville/Maple have no
// pin yet, so they are never auto-picked -- add them here if needed.
...
// Current local time as HH:MM (default for a new entry's start time).
function dailyLogNowTime(d = new Date()) {
  return `${String(d.getHours()).padStart(2, "0")}:${String(d.getMinutes()).padStart(2, "0")}`;
}

// One entry per date: keeps the first entry for each date and drops later
// ones -- cleans up duplicates earlier versions allowed when Save was
...
function DailyLogPanel({ lang, onClose }) {
  const weekdayNames = WEEKDAY_LABELS[lang] || WEEKDAY_LABELS.en;
  // Local date (localDateStr is defined further down, used by the swap finder
  // too). toISOString() was UTC, which in Ontario rolls over to tomorrow
  // every evening after 8 PM.
  ...
  const [startTime, setStartTime] = useState(dailyLogNowTime());
  ...
  const [locStatus, setLocStatus] = useState(null); // { kind, yard? }
  const endYardTouched = React.useRef(false);
    ...
    try {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          setLocStatus({ kind: "ok", yard: hit.yard });
        },
        () => setLocStatus({ kind: "denied" }),
        { enableHighAccuracy: true, timeout: 10000, maximumAge: 60000 }
      );
    } catch { setLocStatus({ kind: "denied" }); }
  };
